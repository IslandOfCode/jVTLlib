...
//ogni riga pu� essere un assegnamento, una definizione, una put o, per il momento, istruzioni di debug personali.
...
/*
 * L'assegnamento � composto da:
 *	varname: il nome della variabile che ospiter� i risultati.
...
/*
 * Questa � una delle parti pi� importanti: l'espressione
...
expr : op=(NOT | PLUS | MINUS) right=expr						#unaryexpr
	 ...
	 //le divido perch� +,* non bisogna scambiare gli operandi
	 | expr op=(PLUS | MUL) expr								#AddMulExpr
	 //qui invece la posizione conta.
	 | expr op=(MINUS | DIV) expr								#MinDivExpr
	 //separo round e ceil/floor perch� funzionano diversamente
	 ...
	 //il NOT? � necessario perch� NOT expr non viene mai riconosciuto
	 | varname (NOT)? IN LPAR literal (COMMA literal)* RPAR			#InCondexpr
	 /* (NOT)* � un hack, visto che (NOT)? non viene assolutamente
	  * considerato. Quindi nel codice devo usare ctx.NOT(0), perch�
...
clausebody : op=(KEEP|RENAME) LPAR clausebodyparam (COMMA clausebodyparam)* RPAR #clauseKeepRename
		   ...
		   //lo separo da keep/ename, perch� questo deve solo aggiungere una/pi� colonna
...
//blocco per calc
//AS � obbligatorio, ROLE opz. Se non specificato, � Measure.
 ...
 * a quanto pare, il +/- va gestito non come token, bens� come operatore unario
 * inoltre, questo deve avere la precedenza pi� alta in assoluto nell'espressione
...
/* Variable */
/*
 * L'ho dovuto piazzare qui sotto per via di un problema di priorit�.
